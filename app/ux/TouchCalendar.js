 ...
 * 
 */
Ext.define('CalendarSencha.ux.TouchCalendar',{
	...
	config: {
		...
		*/
		...
		/**
		* @cfg {Boolean/Object} enableSimpleEvents True to enable the CalendarSencha.ux.TouchCalendarSimpleEvents plugin. When true the CalendarSencha.ux.TouchCalendarSimpleEvents JS and CSS files
		...
		*/
		...
		/**
		* @cfg {Boolean/Object} enableEventBars True to enable the Ext.ux.TouchCalendarEvents plugin. When true the CalendarSencha.ux.TouchCalendarEvents JS and CSS files
		* must be included and an eventStore option, containing an Ext.data.Store instance, be given to the viewConfig.  If an object is passed in this is used as the config for the plugin.
		*/
	...
	*/
	getViewConfig: function(viewValue){
		...
		if(this.getEnableSimpleEvents()){
			...
			plugins.push(Ext.create('CalendarSencha.ux.TouchCalendarSimpleEvents', config));
		} else if (this.getEnableEventBars()){
			var config = Ext.isObject(this.getEnableEventBars()) ? this.getEnableEventBars() : {};
			plugins.push(Ext.create('CalendarSencha.ux.TouchCalendarEvents', config));
		}
	...
	initViews: function(){
		...
		items.push(
		    Ext.create('CalendarSencha.ux.TouchCalendarView', Ext.applyIf({
		        currentDate: viewValue
		...
		// active view
		items.push(
			Ext.create('CalendarSencha.ux.TouchCalendarView', CalendarSencha.ux.TouchCalendarView(this.getViewConfig(origCurrentDate)))
		);

		// second out of view (i.e. third)
		viewValue = this.getViewDate(origCurrentDate, 1);
		items.push(
			Ext.create('CalendarSencha.ux.TouchCalendarView', CalendarSencha.ux.TouchCalendarView(Ext.applyIf({
		        currentDate: viewValue
		    }, this.getViewConfig(viewValue))))
		);
	...
	onActiveItemChange: function(container, newCard, oldCard){
		if (this.getEnableSwipeNavigate()) {
			...
			if (direction === 'forward') {
				...
				var newCalendar = new CalendarSencha.ux.TouchCalendarView(this.getViewConfig(this.getViewDate(newCard.currentDate, 1)));
