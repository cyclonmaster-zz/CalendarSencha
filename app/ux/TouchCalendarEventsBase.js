/**
 * CalendarSencha.ux.TouchCalendarEventsBase
 */
Ext.define('CalendarSencha.ux.TouchCalendarEventsBase', {
	...
	generateEventBars: function(){
		 ...
		 * with the CalendarSencha.ux.CalendarEventBarModel model.
		 * @private
		 */
			...
			eventStore.each(function(event){
				...
				if (eventBarIndex > -1) {
					...
					if (currentDate.getDay() === this.getCalendar().getWeekStart()) {
						...
						var wrappedEventBarRecord = Ext.create('CalendarSencha.ux.CalendarEventBarModel', {
					...
					eventBarRecord = Ext.create('CalendarSencha.ux.CalendarEventBarModel', {
						EventID: event.internalId,
						Date: currentDate,
						BarLength: 1,
						BarPosition: barPos,
	 ...
	 * @method
	 * @private
	 * @param {CalendarSencha.ux.CalendarEventBarModel} r The EventBar model instance to figure out if it wraps to the next row of dates
	 */
	 ...
	 * @method
	 * @private
	 * @param {CalendarSencha.ux.CalendarEventBarModel} r The EventBar model instance to figure out if it has wrapped from the previous row of dates
	 */
